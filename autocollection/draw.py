...
df = pd.read_csv('ultra_close_04-15.csv')
subdf = df[['location','image_url']]
for root, dirs, files in os.walk('ultra_close'):
    for file in tqdm(files):
        ...
        save_path = file.replace('ultra_close','ultra_close_draw')
        save_dir = "/".join(save_path.split('/')[:-1])
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        cv2.imwrite(save_path,img_draw)

        save_path_crop = file.replace('ultra_close','ultra_close_crop')
